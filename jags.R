model <- function ()
{
  ...
  ## Survival part
    ...
    for(k in 1:K)
    {
       ...
       betas.survX0[i, k] <- Bs.betaX0[1:ncF] %*% GammaBs.surv[K * (i - 1) + k, 1:ncF] 
       betas.survX1[i, k] <- Bs.betaX1[1:ncF] %*% GammaBs.surv[K * (i - 1) + k, 1:ncF]
       betas.survX2[i, k] <- Bs.betaX2[1:ncF] %*% GammaBs.surv[K * (i - 1) + k, 1:ncF]
       
       betas.survZ1[i, k] <- Bs.betaZ1[1:ncF] %*% GammaBs.surv[K * (i - 1) + k, 1:ncF]
       betas.survZ2[i, k] <- Bs.betaZ2[1:ncF] %*% GammaBs.surv[K * (i - 1) + k, 1:ncF]
       ...
       prob_surv[i, k] <- 1/(1+exp(-(betas.survX0[i, k] + betas.survX1[i, k]*x1[i] + betas.survX2[i, k]*x2[i] 
                                     + betas.survZ1[i, k]*z1[i] + betas.survZ2[i, k]*z2[i] + b[i] + xi[fac.id[i]])))
       
       log.h0.s[i, k] <- Bs.h0[1:ncF] %*% GammaBs.surv[K * (i - 1) + k, 1:ncF]
       f.long.surv[i, k] <- Bs.alpha[1:ncF]%*% GammaBs.surv[K * (i - 1) + k, 1:ncF]
       
       surv[i, k] <- exp(log.h0.s[i, k] + f.long.surv[i, k] * prob_surv[i, k]  + gamma.survX1[i, k]*x2[i] + gamma.survX2[i, k]*x1[i] 
